...
class CommodityMatchingPolicy(Base):
    ...
    # Line-total (unit_price * quantity) ceiling below which a three-way-match PO
    # line auto-receives even if `auto_receive` itself is False for this scope --
    # e.g. "always auto-receive line items under $500" without having to flip
    # auto_receive for the whole commodity scope. Either condition can trigger
    # auto-receive; see app.services.procurement_workflow.auto_create_receipts_for_ordered_po.
    auto_receive_price_threshold: Mapped[Decimal | None] = mapped_column(Numeric(12, 2), nullable=True)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    updated_by: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
